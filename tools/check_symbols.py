...
It exits non-zero when it finds any of:
  ...
  D. The same physical symbol (address) declared with different names across
     the splat configs (symbols.* / undefined_syms*.*).
...
group*: the main binary (symbols.main.txt) plus, for each overlay, that
...
def check_name_consistency():
    ...
    groups = {}
    # main binary
    groups['main'] = [os.path.join(CFG_DIR, 'symbols.main.txt')]
    for ovl in overlay_names():
        ...
        files.append(os.path.join(CFG_DIR, 'symbols.main.txt'))
        groups[ovl] = files
