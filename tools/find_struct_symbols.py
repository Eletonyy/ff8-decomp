...
4.  Parse ``config/symbols.*.txt`` + ``build/splat/undefined_syms_auto.*.txt`` -> name->addr.
...
# splat-generated symbol files (regenerated by `make split`)
GEN_DIR = os.path.join(ROOT, "build", "splat")
...
def overlay_of(source):
    m = re.search(r"(?:undefined_syms_auto|symbols)\.(\w+)\.txt", source)
...
def main():
    ...
    sym_files = sorted(
        glob.glob(os.path.join(GEN_DIR, "undefined_syms_auto.*.txt"))
        + glob.glob(os.path.join(CONFIG, "symbols.*.txt")))
